...
if miss_tab.empty:
    ...
    def little_mcar_test(data):
        ...
        for _, pattern in patterns.iterrows():
            ...
            idx_c = [all_cols.index(c) for c in obs_cols] #indoce de covarianza
    ...
    if pval_little is not None:
        ...
        if pval_little > 0.05:
            print(" -> MCAR confirmado: imputamos por moda (variables categóricas ordinales, n_miss ≤ 4)")
        else:
...
print(f"""
  ...
  y tienen mayor interpretabilidad médica directa validadada.
""")
...
"""
  ...
  Algoritmos comparados:
    A) PC    — constraint-based (χ², α=0.15)
    B) HC-BIC — score-based greedy (BIC)
    C) HC-BDeu — score-based greedy (BDeu, ESS=5)
    D) MMHC  — híbrido (PC skeleton + HC-BIC)
  De los automáticos se obtiene la red común (consenso estadístico).
  La red experta se construye con conocimiento clínico bibliográfico.
"""
...
def viz_grafo_completo(model, titulo, ax, vars_cont=None, target=TARGET):
    ...
    nx.draw_networkx(G, pos=pos, ax=ax, node_color=cmap, node_size=800,
                     font_size=6, font_color='white', arrows=True,
                     arrowsize=13, edge_color='#444', width=1.1)
...
plt.suptitle('UCI Cleveland: Red Experta vs Red Común Completa', fontsize=13)
plt.tight_layout()
...
fig2, axs2 = plt.subplots(nr, nc, figsize=(14, 5 * nr))
...
plt.suptitle('UCI Cleveland: RB automáticas', fontsize=10)
plt.tight_layout()
plt.savefig('uci_automaticas.png', dpi=120, bbox_inches='tight')
plt.close()
print(f"\n  Red experta: {len(list(model_expert.edges()))} aristas, "
      f"{len(list(model_expert.nodes()))} nodos")
if red_comun:
    print(f"  Red común:   {len(list(red_comun.edges()))} aristas, "
          f"{len(list(red_comun.nodes()))} nodos")
...
class RedBayesianaHibridaCLG:
    ...
    def p_cont(self, nodo, x, pa_disc=None):
        """
        Calcula la ensidad gaussiana f(x | padres_disc=pa_disc), siendo
...
def estimar_cpts_ess5(model, nombre, ess=5):
    """
    Esta función estiima las CPTs de los nodos discretos con BayesianEstimator ESS=5.
...
for row_i, m in enumerate([modelo_expert, modelo_comun]):
    for col_i, col_c in enumerate(VARS_CONTINUAS_RB):
        ...
        if pa_disc_c and col_c in m.clg:
            ...
            ax.set_title(f"{m.nombre}: {col_c} | {grp_col}", fontsize=8)
        ...
        ax.set_xlabel(col_c, fontsize=7)
...
r_target = _card(modelo_expert, TARGET)  # nº estados de target 
 
for nombre_m, m in [('Experta', modelo_expert), ('Común', modelo_comun)]:
    print(f"\n  ── {nombre_m} ──────────────────────────────────────────")
    print(f"  {'Perfil':<42} {'Método':>5}  "
          f"{'P(enf>0)':>8}  {'MAP':>3}  "
          f"{'k0':>6}{'k1':>6}{'k2':>6}{'k3':>6}{'k4':>6}")
    print("  " + "─" * 85)
    for desc, ev_d, ev_c in PERFILES: # cada paciente
        ...
        for metodo, fn in [('VE', ve_hibrido), ('BP', bp_hibrido)]: #inferencia exacta
            try:
                dist = fn(m, TARGET, ev_d_f, ev_c_f) #P(TARGET∣evidencia)
                dist = np.array(dist, dtype=float)
                if len(dist) < r_target: # Aseguramos longitud correcta
                    dist = np.pad(dist, (0, r_target - len(dist)))
                dist = dist[:r_target]
                z = dist.sum()
                dist = dist / z if z > 1e-300 else np.ones(r_target)/r_target #dist.valida
                # métricas clínicas
                p_enf = float(1 - dist[0]) # P(enfermedad)=1−P(estado 0)
                mapa = int(np.argmax(dist)) # estado más probable: arg maxk ​P(X=k∣e)
                ks = "".join([f"{v:6.3f}" for v in dist])
                label = desc if metodo == 'VE' else ''
                print(f"  {label:<42} {metodo:>5}  "
                      f"{p_enf:>8.4f}  {mapa:>3}  {ks}")
            except Exception as e:
                print(f"  {desc:<42} {metodo:>5}  Error: {e}")
...
print("\n--- 6.4 Actualización secuencial de P(enf>0) ---")
"""
  Simulación de una consulta médica: se añade evidencia progresivamente
  y se observa cómo actualiza P(enf>0) = P(target>0 | evidencia).
...
print(f"\n  {'Paso':<42} {'Experta VE':>12} {'Común VE':>10}  Distribución (Experta)")
print("  " + "─" * 85)
for desc, ev_d, ev_c in SECUENCIA:
    for nombre_m, m in [('Experta', modelo_expert), ('Común', modelo_comun)]:
        ...
        try:
            ...
            p_enf = float(1 - dist[0])
            if nombre_m == 'Experta':
                ks = " ".join([f"k{k}:{v:.3f}" for k, v in enumerate(dist)])
                print(f"  {desc:<42} {p_enf:>12.4f}", end="")
                _ks_exp = ks
            else:
                print(f" {p_enf:>10.4f}  {_ks_exp}")
        except Exception as e:
            if nombre_m == 'Común':
                print(f" {'Error':>10}  {e}")
...
for nombre_m, m in [('Experta', modelo_expert), ('Común', modelo_comun)]:
    ...
    dist_ve /= dist_ve.sum()
    p_ve_b7 = float(1 - dist_ve[0])
    print(f"  {nombre_m:<10} A: P(enf>0)={p_ve_b7:.6f}  [{t_ve:.1f} ms]  "
          + "  ".join([f"k{k}:{dist_ve[i]:.3f}"
                       for i, k in enumerate(_estados(m, TARGET))]))
    resultados_b7[f'VE_{nombre_m}'] = {'p_enf': p_ve_b7, 'dist': dist_ve, 'ms': t_ve}
    # Ejemplo B 
    ...
    p_ve_b7_2 = float(1 - dist_ve2[0])
    print(f"  {nombre_m:<10} B: P(enf>0)={p_ve_b7_2:.6f}  [{t_ve2:.1f} ms]  "
          + "  ".join([f"k{k}:{dist_ve2[i]:.3f}"
                       for i, k in enumerate(_estados(m, TARGET))]))
    resultados_b7[f'VE_{nombre_m}_cont'] = {
        'p_enf': p_ve_b7_2, 'dist': dist_ve2, 'ms': t_ve2}
...
def prior_sample_clg(m, rng):
    ...
    """
    estado = {}
    for nodo in m.topological_order(): # recorre la red en orden topologico (padres se generan antes que hijoa)
...
for nombre_m, m in [('Experta', modelo_expert), ('Común', modelo_comun)]:
    ...
    p_ve_ref = resultados_b7[f'VE_{nombre_m}']['p_enf']
    ...
    if cadenas:
        ...
        cuentas = {k: float((todas == k).mean()) for k in estados_tgt}
        p_gibbs = float(1 - cuentas.get(estados_tgt[0], 0))
        ...
        dist_str = "  ".join([f"k{k}:{cuentas.get(k, 0):.3f}" for k in estados_tgt])
        print(f"  Gibbs {nombre_m}: P(enf>0)={p_gibbs:.6f}  "
              f"Error={abs(p_gibbs-p_ve_ref):.4f}  [{t_g:.0f} ms]")
        print(f"  Distribución: [{dist_str}]")
        print(f"  {conv}")
        tabla_b7.append({'Método': f'Gibbs ({N_CHAINS}×{N_SAMPLES})',
                         'Red': nombre_m, 'P(enf>0)': round(p_gibbs, 5),
                         'Error vs VE': round(abs(p_gibbs-p_ve_ref), 5),
                         'ms': round(t_g, 0)})
        resultados_b7[f'Gibbs_{nombre_m}'] = {
            'p_enf': p_gibbs, 'cadenas': cadenas, 'r_hat': r_hat, 't_ms': t_g}
    else:
...
for nombre_m, m in [('Experta', modelo_expert), ('Común', modelo_comun)]:
    ...
    estados_tgt = _estados(m, TARGET)
    print(f"\n  ── {nombre_m} ──")
    print(f"  {'N':>7}  {'P(enf>0)':>10}  {'Error vs VE':>12}  {'ms':>6}")
    print("  " + "─" * 42)
    p_lw_list = []
    for n_s in NS_LW:
        t0 = time.time()
        try:
            ...
            p_lw = float(1 - dist_lw[0])
        except Exception as e_lw:
            print(f"  [LW error N={n_s}]: {e_lw}")
            p_lw = float('nan')
        t_ms = (time.time() - t0) * 1000
        p_lw_list.append(p_lw)
        err  = abs(p_lw - p_ve_ref) if not np.isnan(p_lw) else float('nan')
        print(f"  {n_s:>7}  {p_lw:>10.5f}  {err:>12.5f}  {t_ms:>6.0f}")
        tabla_b7.append({'Método': f'LW (N={n_s})', 'Red': nombre_m,
                         'P(enf>0)': round(p_lw, 5),
                         'Error vs VE': round(err, 5), 'ms': round(t_ms, 0)})
    resultados_b7[f'LW_{nombre_m}'] = {'p_lw_list': p_lw_list, 'ns': NS_LW}


# 7.4 - Gráficas:
colores_k = ['#4e79a7', '#76b7b2', '#f28e2b', '#e05c4b', '#59a14f']
fig7, axes7 = plt.subplots(3, 2, figsize=(16, 15))
...
for col_i, (nombre_m, m) in enumerate([('Experta', modelo_expert),
    ...
    # Convergencia LW vs VE + Gibbs como referencia
    ax_lw = axes7[0, col_i]
    ...
    ax_lw.axhline(p_ve_ref, color='#e05c4b', linestyle='--', lw=2,
                  label=f'VE exacto = {p_ve_ref:.4f}')
    if 'p_enf' in gib_data_m:
        p_g = gib_data_m['p_enf']
        r_g = gib_data_m.get('r_hat', float('nan'))
        ax_lw.axhline(p_g, color='#f28e2b', linestyle=':', lw=2,
                      label=f'Gibbs = {p_g:.4f}  (R̂={r_g:.3f})')
    ax_lw.set_xlabel('N muestras LW (log)')
    ax_lw.set_ylabel('P(enf>0 | ev)')
    ax_lw.set_title(f'Red {nombre_m}: convergencia LW vs VE')
    ax_lw.legend(fontsize=8); ax_lw.grid(alpha=0.3)
    ...
    ax_acf.legend(fontsize=7); ax_acf.grid(alpha=0.3)
    ...
    ax_evo.set_title(f'Red {nombre_m} -> evolución P(target=k): cadena 1\n'
                     f'(punteado = VE exacto, sin ventana)')
    ax_evo.legend(fontsize=5, ncol=2); ax_evo.grid(alpha=0.3)
...
for nombre_rb, y_pred_rb, y_prob_rb in [
    ('RB Experta (VE)', np.array(y_pred_rb_exp), y_prob_rb_exp),
    ('RB Común  (VE)', np.array(y_pred_rb_com), y_prob_rb_com),
]:
...
fig_roc, axes_roc = plt.subplots(1, n_plot, figsize=(4 * n_plot, 4))
if n_plot == 1:
    axes_roc = [axes_roc]
for i_c, clase in enumerate(clases[:n_plot]):
    ...
    for nombre, res in resultados_ml.items():
        ...
        ax_r.plot(fpr, tpr,
                  ...
                  lw=1.8, label=f"{nombre} ({roc_auc_c:.2f})")
    ax_r.plot([0, 1], [0, 1], 'k--', lw=0.8)
    ax_r.set_xlabel('FPR'); ax_r.set_ylabel('TPR')
    ax_r.set_title(f'ROC: target={clase}\n(OvR)')
    ax_r.legend(fontsize=6); ax_r.grid(alpha=0.3)
 
plt.suptitle('Curvas ROC por clase (One-vs-Rest): ML vs RB híbrida', fontsize=8)
plt.tight_layout()
...
print("\n--- 9.1 Permutation Importance (RF y XGBoost) ---")

perm_results = {}
for nombre_clf in ['Random Forest', 'XGBoost']:
    clf = resultados_ml[nombre_clf]['clf']
    ...
    perm_results[nombre_clf] = {'mean': pi_mean, 'std': pi_std,
                                 'raw': perm.importances}
    pi_sorted = pi_mean.sort_values(ascending=False)
    print(f"\n  {nombre_clf} (balanced_accuracy):")
    print(f"  {'Variable':<14} {'Δbal_acc':>12}  {'±std':>8}")
    print("  " + "─" * 38)
...
for nombre_m_s in ['Experta', 'Común']:
    ...
    print(f"  {'Variable':<14} {'Tipo':<8} {'ΔP(enf>0)':>10}  "
...
try:
    ...
    axes_shap[0].set_title('SHAP: importancia global\n'
                            '(media |SHAP| sobre todas las clases)')
    ...
    try:
        ...
        axes_shap[1].set_title(f'SHAP dot: target=1  [{e_bee}]')
 
    plt.suptitle('SHAP values — Random Forest (multiclase OvR)',
                 fontsize=12, y=1.01)
    plt.tight_layout()
...
axes9[0].set_xlabel('Δbalanced_accuracy (permutation)')
axes9[0].set_title('RF: Permutation Importance\n(F1-macro, ±std, 30 repeticiones)')
...
if sens_com:
    ...
    ax9c.barh(sens_ser_c.index, sens_ser_c.values, color=cols_c, alpha=0.85)
    ax9c.legend(handles=[
        mpatches.Patch(color='#f28e2b', label='Discreta'),
        mpatches.Patch(color='#e05c4b', label='Continua'),
    ], fontsize=8, loc='lower right')
ax9c.set_xlabel('ΔP(enf>0)')
ax9c.set_title('RB Común: sensibilidad marginal\n(naranja=discreta, rojo=continua)')
ax9c.grid(alpha=0.3, axis='x')
...
for row_i, (nombre_m, m) in enumerate([('Experta', modelo_expert),
    ...
    ax_l.set_xlabel('thalach (lpm)'); ax_l.set_ylabel('Densidad')
    ax_l.set_title(f'Red {nombre_m}: P(thalach|target=k)')
    ...
    ax_r.set_xlabel('thalach (lpm)'); ax_r.set_ylabel('P(target=k | thalach=t)')
    ax_r.set_title(f'Red {nombre_m}: inversión bayesiana CLG')
...
plt.suptitle('Análisis CLG: P(thalach|target) y P(target|thalach)\n'
             'Experta (fila sup.) vs Común (fila inf.)', fontsize=11)
plt.tight_layout()
...
#  10.4 Robustez: dataset reducido (fbs disc + trestbps cont) 
...
ax1.set_xlabel('% vars. discretas con NaN')
ax1.set_ylabel('Accuracy (test)')
ax1.set_title('A) NaN discretos\n(RB marginaliza, RF imputa)')
...
ax2.set_xlabel('σ ruido gaussiano (unidades originales)')
